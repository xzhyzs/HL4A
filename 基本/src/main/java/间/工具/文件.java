...
public class 文件 {
    ...
    public static Boolean 是文件(String... $文件) {
        return 取文件对象(字符.分解($文件, "/")).isFile();
    }

    public static Boolean 是目录(String... $文件夹) {
        return 取文件对象(字符.分解($文件夹, "/")).isDirectory();
    }

    public static Boolean 存在(String $路径) {
    ...
    private static 有序哈希表 前缀替换 = new 有序哈希表();
    ...
    public static void 删除关键字(String $地址,有序哈希表 $关键字) {
        File $对象 = 取文件对象($地址);
        if ($对象.exists())
