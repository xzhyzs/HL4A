...
public class 用户 extends AVUser {
    ...
    private Bitmap 取头像缓存() {
        if (!头像.检查(this) || 头像.读取(this).读取() == null) {
            头像.设置(this,new 引用<Bitmap>(图片.读取(头像缓存 + "/" + getObjectId())));
        }
        return 头像.读取(this).读取();
    }
    ...
    public void 检查(final 方法 $回调) {
        isAuthenticated(new AVCallback<Boolean>() {
                @Override
                protected void internalDone0(Boolean $返回,AVException avException) {
