...
public final class 文本实现 {
    ...
    public static void 插入图片(final TextView $视图, final String $文本, final Bitmap $图片) {
        $视图.addTextChangedListener(new TextWatcher() {
                @Override
                ...
                public void onTextChanged(CharSequence $内容, int p2, int p3, int p4) {
                    if (字符.是否出现($内容.toString(),$文本)) {
                        ImageSpan $替换 = new ImageSpan($视图.getContext(), $图片);
